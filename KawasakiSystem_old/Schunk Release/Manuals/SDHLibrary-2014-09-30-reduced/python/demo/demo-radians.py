...
#    See demo-radians.__doc__ and online help ("-h" or "--help") for available options.
#
...
#######################################################################
## \anchor sdhlibrary_python_demo_radians_python_vars
...
__version__   = "$Id: demo-radians.py 11045 2013-11-27 15:12:49Z Osswald2 $"
__copyright__ = "Copyright (c) 2007 SCHUNK GmbH & Co. KG"

#  end of doxygen name group sdhlibrary_python_demo_radians_python_vars
#  @}
######################################################################
...
# make sure that radians are used:
options.use_radians = True

# Create an instance "hand" of the class cSDH:
hand = sdh.cSDH( options=options.__dict__ )
dbg << "Successfully created cSDH instance using radians\n"  # pylint: disable-msg=W0104
...
try:
    ...
    hand.SetAxisTargetVelocity( sdh.All, [ 40.0*math.pi/180.0 ] * 7 )
    ...
    fta = list(faa) 
    fta[1] -= 10*math.pi/180.0
    fta[2] -= 10*math.pi/180.0
